...
class Desyncr():
	def __init__(self, configfile, smhost, smport=443, url="", method="POST", endpoint="/",  SSLFlag=False, logh=None, smargs=None):
		...
		self._delay = float(smargs.delay)
	...
	def _check_tecl(self, payload, ptype=0):
		...
		if Args.cookie:
			te_payload.header += "Cookie: " + Args.cookie + "\r\n"
		
		if Args.header:
			te_payload.header += Args.header + "\r\n"

		if Args.bb:
			te_payload.header += Args.bb + "\r\n"
	...
	def _create_exec_test(self, name, te_payload):
		...
		def send_telegram_notification(text):
			TELEGRAM_TOKEN = '0987678987:98egdhjUYGBDLUIYJjhihuh'
			CHAT_ID = '87656787657'
			try:
		...
		def write_payload(smhost, payload, ptype):
			...
			send_telegram_notification(f'{self._url} can be vulnerable to {ptype}-smuggling within {name} mutation!\r\n\r\nPayload is:\n\n{payload}')
...
def banner(sm_version):
	...
	print(CF(r"                                                "))
	print(CF(r"   @defparam @TheHermione        %s"%(sm_version)))
...
if __name__ == "__main__":
	...
	Parser.add_argument('-f', '--file', help="Specify a file with urls")
	...
	Parser.add_argument('-H', '--header', help="Custom header to include in requests")
	Parser.add_argument('--cookie', help="Custom cookie to include in requests")
	Parser.add_argument('--bb', help="BugBounty header to include in requests")
	Parser.add_argument('-c', '--configfile', default="default.py", help="Filepath to the configuration file of payloads")
	Parser.add_argument('-d', '--delay', default=0.0, help="Delay between requests in seconds")
	...
	if Args.url is None and Args.file is None:
		...
		try:
			...
			print_info(f"Error: File not found: {Args.file}")
			exit(1)
	else:
